...
let getResponse port txd : Result<byte[], string>=         
    try
        match tryApplyPort port with
        | Err error -> formatError port.CanLog port.PortName error txd |> Err
        | Ok serial ->
            loopTransmit { config = port; serial = serial; n =  0; txd =  txd }            
    with e ->
        formatError port.CanLog port.PortName e.Message txd |> Err
...
module Protocol = 
    ...
    let tell (whatProtocol, port, trigger) request rxd formatResult (result : _) : _ =
        let level, strResult, needLog = 
            match result with
            | Ok x -> Logging.Info, formatResult x, port.CanLog || request.canLog 
            | Err e -> Logging.Error, e, true
        
        trigger ( request, level, rxd, strResult  )
        if needLog then             
            Logging.write level "%s %s адр.%d, %s, ком.%d%s" 
                port.PortName whatProtocol request.addy request.what request.cmd 
                (if strResult = "" then "" else " ==> " + strResult )                 
        result
        

    let getResponse ((_, port, _ ) as proto) (makeTxD, checkRxD) request formatResult parseRxD =
        let rxd, result = 
            match getResponse port (makeTxD request) with
            | Err error -> None, Err error
            | Ok rxd -> Some rxd, checkRxD request rxd  |> Result.bind parseRxD
        tell proto request rxd formatResult result